...
import com.apollographql.apollo3.ksp.codegen.KotlinSymbols.AstEnumValue
import com.apollographql.apollo3.ksp.codegen.KotlinSymbols.AstValue
...
) : com.apollographql.apollo3.ksp.codegen.CgFileBuilder {
  ...
  override fun build(): FileSpec {
    ...
    return FileSpec.builder(packageName = context.packageName, fileName = fileName)
        .apply {
          ...
          sirEnumDefinitions.map { it.propertySpec() } + sirInputObjectDefinitions.map { it.propertySpec() }
              .forEach {
                addProperty(it)
              }
        }
